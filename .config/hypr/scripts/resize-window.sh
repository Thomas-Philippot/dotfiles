...
handle() {
    case $1 in
        windowtitle*)
            ...
            if [[ "$window_title" == *"Extension : (Gestionnaire de mots de passe Bitwarden) - Bitwarden — Mozilla Firefox"* || "$window_title" == "Coinbase Wallet" ]]; then
                ...
                hyprctl --batch "dispatch togglefloating address:0x$window_id; dispatch resizewindowpixel exact 14% 54%,address:0x$window_id; dispatch centerwindow"
            fi
            ;;
    esac
}
